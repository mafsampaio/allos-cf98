# Deploy 24/7 do Allos no Linux (systemd + tmux + Cloudflare Named Tunnel)
...
| Serviço | Função | Tipo |
|---|---|---|
| `allos-webhook.service` | webhook_server.py escutando em `:3020` | long-running |
| `allos-tunnel.service` | Cloudflare Named Tunnel expõe `:3020` num subdomínio fixo | long-running |
| `allos-monitor.service` | cria sessão tmux `allos` rodando `claude` em loop | oneshot+remain |
...
1. **Allos já instalado e funcionando manualmente** (siga o `INSTALL_PROMPT.md` antes). O `python -m whatsapp_agent.doctor` deve estar todo verde.
...
```bash
cloudflared tunnel create allos
```
...
```bash
cloudflared tunnel delete allos
cloudflared tunnel create allos
```

### 2.3 Apontar o subdomínio pro tunnel

Substitua `allos.seudominio.com` pelo subdomínio que você quer:

```bash
cloudflared tunnel route dns -f allos allos.seudominio.com
```
...
### 2.5 Atualizar `config.py` do Allos

Edite `config.py` e fixe a URL pública:

```python
PUBLIC_WEBHOOK_URL = "https://allos.seudominio.com"
```
...
#!/usr/bin/env bash
set -u
cd /home/SEU_USER/Allos
source /home/SEU_USER/Allos/.venv/bin/activate

PROMPT='Leia CLAUDE_PROMPT.md e execute o prompt do passo 2. SESSAO: 1'
FIRST_RUN_FLAG=/home/SEU_USER/Allos/.claude_monitor_started
...
### 3.2 `scripts/tmux_ensure_allos.sh`
...
if tmux has-session -t "$SESSION" 2>/dev/null; then
    echo "[tmux_ensure_allos] session '$SESSION' already running"
    exit 0
fi
echo "[tmux_ensure_allos] creating tmux session '$SESSION'"
tmux new-session -d -s "$SESSION" -c /home/SEU_USER/Allos \
    "/home/SEU_USER/Allos/scripts/claude_monitor_loop.sh"
echo "[tmux_ensure_allos] attach with: tmux attach -t $SESSION"
```

### 3.3 Permissões

```bash
chmod +x scripts/claude_monitor_loop.sh scripts/tmux_ensure_allos.sh
```
...
### 4.1 `allos-webhook.service`

```ini
[Unit]
Description=Allos WhatsApp webhook server (port 3020)
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
WorkingDirectory=/home/SEU_USER/Allos
ExecStart=/home/SEU_USER/Allos/.venv/bin/python -m whatsapp_agent.webhook_server
Restart=on-failure
RestartSec=5
StandardOutput=append:/home/SEU_USER/Allos/webhook.log
StandardError=append:/home/SEU_USER/Allos/webhook.log

[Install]
WantedBy=default.target
```

### 4.2 `allos-tunnel.service`

```ini
[Unit]
Description=Allos Cloudflare Named Tunnel
After=network-online.target allos-webhook.service
Wants=network-online.target
Requires=allos-webhook.service

[Service]
Type=simple
WorkingDirectory=/home/SEU_USER/Allos
ExecStart=/usr/local/bin/cloudflared tunnel --no-autoupdate --config /home/SEU_USER/.cloudflared/config.yml run allos
Restart=on-failure
RestartSec=10
StandardOutput=append:/home/SEU_USER/Allos/cloudflared.log
StandardError=append:/home/SEU_USER/Allos/cloudflared.log

[Install]
WantedBy=default.target
```

### 4.3 `allos-monitor.service`

```ini
[Unit]
Description=Allos Claude Code monitor (tmux session 'allos')
After=allos-tunnel.service
Wants=allos-tunnel.service

[Service]
Type=oneshot
RemainAfterExit=yes
WorkingDirectory=/home/SEU_USER/Allos
ExecStart=/home/SEU_USER/Allos/scripts/tmux_ensure_allos.sh
ExecStop=/usr/bin/tmux kill-session -t allos
...
systemctl --user enable --now allos-webhook.service allos-tunnel.service allos-monitor.service
```

Verifique:

```bash
systemctl --user status allos-webhook allos-tunnel allos-monitor
```
...
```bash
curl -I https://allos.seudominio.com
...
journalctl --user -u allos-webhook -u allos-tunnel -u allos-monitor -f

# Por serviço
journalctl --user -u allos-webhook -f
journalctl --user -u allos-tunnel -f
journalctl --user -u allos-monitor -f

# Logs de aplicação
tail -f /home/SEU_USER/Allos/webhook.log         # mensagens chegando
tail -f /home/SEU_USER/Allos/cloudflared.log     # tunnel
tail -f /home/SEU_USER/Allos/raw_debug.jsonl     # payload bruto megaAPI (verboso)
tail -f /home/SEU_USER/Allos/messages_session1.jsonl  # mensagens já filtradas
```
...
```bash
# Últimas 100 linhas
journalctl --user -u allos-webhook -n 100 --no-pager

# Desde 1h atrás
journalctl --user -u allos-tunnel --since "1 hour ago"

# Apenas erros
journalctl --user -u allos-monitor -p err
```
...
```bash
# Reiniciar tudo
systemctl --user restart allos-webhook allos-tunnel allos-monitor

# Parar tudo
systemctl --user stop allos-monitor allos-tunnel allos-webhook

# Iniciar tudo
systemctl --user start allos-webhook allos-tunnel allos-monitor

# Status agregado
systemctl --user status allos-webhook allos-tunnel allos-monitor

# Diagnóstico do agente
cd ~/Allos && .venv/bin/python -m whatsapp_agent.doctor
```

### Aliases úteis pro `~/.bashrc`

```bash
alias allos-logs='journalctl --user -u allos-webhook -u allos-tunnel -u allos-monitor -f'
alias allos-status='systemctl --user status allos-webhook allos-tunnel allos-monitor'
alias allos-restart='systemctl --user restart allos-webhook allos-tunnel allos-monitor'
alias allos-stop='systemctl --user stop allos-monitor allos-tunnel allos-webhook'
alias allos-start='systemctl --user start allos-webhook allos-tunnel allos-monitor'
alias allos-msgs='tail -f ~/Allos/messages_session1.jsonl'
alias allos-raw='tail -f ~/Allos/raw_debug.jsonl'
alias allos-webhook-log='tail -f ~/Allos/webhook.log'
alias allos-tunnel-log='tail -f ~/Allos/cloudflared.log'
alias allos-attach='tmux attach -t allos'
alias allos-doctor='cd ~/Allos && .venv/bin/python -m whatsapp_agent.doctor'
```
...
```bash
systemctl --user restart allos-monitor
```
...
Cria/edita `~/.claude/settings.json` (global) ou `Allos/.claude/settings.json` (por projeto):
...
Anexa no tmux (`tmux attach -t allos`) e olha o canto inferior direito da interface — mostra o modelo ativo. Ou no claude rodando:
...
- Monitor tool não foi iniciado: o prompt inicial pode não ter sido executado corretamente — delete o flag e reinicie:
  ...
  ```bash
  rm -f ~/Allos/.claude_monitor_started
  systemctl --user restart allos-monitor
  ```
...
### URL `allos.seudominio.com` não resolve
...
- **`Requires=allos-webhook.service` no tunnel**: garante que tunnel só sobe se webhook estiver vivo (sem isso, você teria janela onde o domínio responde 502)
...
- [ ] `curl https://allos.seudominio.com` retorna algo (não DNS error)
- [ ] `systemctl --user status` mostra os 3 serviços ativos
- [ ] `tmux attach -t allos` mostra `claude` rodando com Monitor tool ativo
...
- Repositório: https://github.com/giovani-junior-dev/Allos
