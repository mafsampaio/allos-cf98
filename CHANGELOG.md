...
### Added
- **Trilha 2 promoted to official production path.** `docs/DEPLOY_24_7_LINUX.md` is now the canonical 24/7 runbook (tested on Ubuntu 24.04). It uses three systemd **user** services — `allos-webhook.service`, `allos-tunnel.service`, `allos-monitor.service` — plus a tmux session running `claude --dangerously-skip-permissions --continue` in a loop, and a Cloudflare Named Tunnel for a stable public URL.
...
[Unreleased]: https://github.com/giovani-junior-dev/Allos/compare/v1.2.0...HEAD
[1.2.0]: https://github.com/giovani-junior-dev/Allos/releases/tag/v1.2.0
