...
Usage:
    python discover_lid.py [session]   (default session: 1)

Steps for the user:
    1. Make sure webhook_server.py + ngrok are running (./start.ps1 or .sh)
    2. Run this script
...
def find_lid_for_phone(phone: str, since_byte: int) -> tuple[str, int]:
    ...
    with open(RAW_FILE, encoding="utf-8") as f:
        ...
        for line in f:
            ...
            for candidate in (jid, participant):
                if not candidate:
                    continue
                # match @lid format
                m = re.match(r"^(\d+)@lid$", candidate)
                if m:
                    return m.group(1), f.tell()
                # match @s.whatsapp.net with matching phone -> grab participant @lid if exists
                m2 = re.match(r"^(\d+)@s\.whatsapp\.net$", candidate)
                if m2 and m2.group(1) == phone and participant.endswith("@lid"):
                    return participant.replace("@lid", ""), f.tell()
        return "", f.tell()
...
def main() -> int:
    ...
    print("  - webhook_server.py esta rodando? (python doctor.py)")
    print("  - URL ngrok configurada no megaAPI?")
    print("  - sua instancia megaAPI esta conectada?")
    return 1
